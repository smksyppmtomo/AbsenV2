...
// Setup and start Carousel transitions
function initCarousel() {
  updateCarousel();

  // Set transition timer (every 3.5 seconds)
...
// Mendaftarkan service worker kosong untuk memenuhi kriteria instalasi PWA standar browser (YouTube/GitHub)
function registerServiceWorker() {
  if ("serviceWorker" in navigator) {
    window.addEventListener("load", () => {
      // Kita membuat Service Worker in-line menggunakan Blob agar aplikasi tetap berada dalam satu file tunggal
      const swCode = `
              ...
              self.addEventListener('fetch', (e) => {
                // Biarkan request lewat langsung ke jaringan
      ...
      navigator.serviceWorker
        .register(swUrl)
        .then((reg) => {
          console.log("Service Worker berhasil didaftarkan secara dinamis!", reg.scope);
        })
        .catch((err) => {
          console.warn("Registrasi Service Worker dilewati (berjalan lokal/preview):", err);
        });
    });
  }
}
...
async function initApp() {
  ...
  // Daftarkan Service Worker PWA agar tombol install native muncul otomatis di browser
