...
let isLoading = false;
let deferredPrompt = null; // Menyimpan event instalasi PWA

// Terakhir kali pemanggilan reverse geocoding untuk throttling
...
const quotes = [
  ...
  { text: "Kedisiplinan adalah jembatan antara tujuan and pencapaian.", author: "Jim Rohn" },
  ...
  { text: "Kegenerasi adalah kesuksesan yang tertunda.", author: "Anonim" },
  ...
  { text: "Jangan tanya apa yang bisa negara berikan padamu, tapi tanyakan apa yang bisa kamu verifikasi untuk negara.", author: "John F. Kennedy" },
  ...
  { text: "Sukses adalah kemandirian untuk melangkah dari satu kegagalan ke kegagalan lain tanpa kehilangan antusiasme.", author: "Winston Churchill" },
  ...
  { text: "Kepercayaannya adalah hal yang paling sulit untuk diperoleh dan paling mudah untuk hilang.", author: "Anonim" },
  ...
  { text: "Keberhasilan adalah bukanlah milik orang pintar, tetapi milik orang yang pantang menyerah.", author: "B.J. Habibie" },
...
let currentSlide = 0;
const totalSlides = 6; // Mengatur 6 Slide secara statis sesuai dengan gambar di HTML
let carouselInterval = null;

// Function to update the slideshow position and indicators
function updateCarousel() {
  ...
  indicators.forEach((indicator, index) => {
    if (index === currentSlide) {
      indicator.className = "w-6 h-2.5 rounded-full bg-orange-500 cursor-pointer transition-all duration-300";
    } else {
      indicator.className = "w-2.5 h-2.5 rounded-full bg-white/50 cursor-pointer transition-all duration-300";
    }
  });
}
...
// Setup and start Carousel transitions (Diperlambat menjadi 6000ms / 6 detik)
function initCarousel() {
  updateCarousel();

  // Set transition timer (setiap 6 detik)
  carouselInterval = setInterval(nextSlide, 6000);

  // Add manual navigation click listeners on indicator dots
  const indicators = document.querySelectorAll("#carousel-indicators span");
  ...
  // Menambahkan Dukungan Usapan (Touch Swipe Gestures) untuk Handphone
  const track = document.getElementById("carousel-track");
  let startX = 0;
  let endX = 0;
  ...
  if (track) {
    ...
    track.addEventListener(
      "touchend",
      () => {
        const threshold = 50; // Jarak usapan minimal dalam pixel
        ...
        updateCarousel();
        // Jalankan kembali interval auto-slide setelah selesai berinteraksi
        carouselInterval = setInterval(nextSlide, 6000);
      },
      { passive: true },
    );
...
// Pendeteksi PWA Instalasi Ala YouTube/GitHub
function setupPWAInstallation() {
  window.addEventListener("beforeinstallprompt", (e) => {
    // Mencegah browser otomatis memunculkan prompt bawaan yang mengganggu
    e.preventDefault();
    // Menyimpan event agar bisa dipicu nanti secara manual
    deferredPrompt = e;
    // Memperlihatkan tombol "Pasang" di header
  ...
  window.addEventListener("appinstalled", (evt) => {
    // Ketika aplikasi sudah sukses diinstal
    showToast("Aplikasi Absensi berhasil dipasang di perangkat Anda!", "success");
    const installBtn = document.getElementById("install-pwa-btn");
    if (installBtn) {
...
// Memicu dialog install native dari sistem operasi
function triggerInstall() {
  if (!deferredPrompt) {
    showToast("Aplikasi sudah terpasang atau tidak didukung di browser ini.", "info");
    return;
  }
  // Munculkan dialog
  deferredPrompt.prompt();
  // Cek keputusan pengguna
  deferredPrompt.userChoice.then((choiceResult) => {
    if (choiceResult.outcome === "accepted") {
      console.log("User menerima proses instalasi");
    } else {
      console.log("User membatalkan proses instalasi");
    }
    deferredPrompt = null;
    const installBtn = document.getElementById("install-pwa-btn");
    if (installBtn) {
...
// Mendaftarkan service worker kosong untuk memenuhi kriteria PWA
function registerServiceWorker() {
  if ("serviceWorker" in navigator) {
    window.addEventListener("load", () => {
      const swCode = `
              const CACHE_NAME = 'absensi-yppm-v1';
      ...
      navigator.serviceWorker
        .register(swUrl)
        .then((reg) => {
          console.log("Service Worker didaftarkan dinamis!", reg.scope);
        })
        .catch((err) => {
          console.warn("Registrasi Service Worker dilewati (lokal/preview):", err);
        });
    });
  }
}
...
async function initApp() {
  ...
  // Load initial weather for SMK YPPM Tomo (Fallback coordinates)
  fetchWeather(-6.764191, 108.132599, "YPPM Tomo");
...
async function fetchWeather(lat, lng, locName) {
  ...
  try {
    ...
    if (data && data.current_weather) {
      ...
      // Mapping WMO Weather Codes to Indonesian texts and emojis
      let desc = "Cerah";
      let icon = "☀️";
      ...
      const wmoCodes = {
        ...
        82: { desc: "Hujan Deras Sangat Lebat", icon: "⛈️" },
        95: { desc: "Badai Petir", icon: "⛈️" },
        96: { desc: "Badai Petir Ringan", icon: "⛈️" },
        99: { desc: "Badai Petir Lebat", icon: "⛈️" },
      };

      if (wmoCodes[code]) {
        desc = wmoCodes[code].desc;
        icon = wmoCodes[code].icon;
      ...
      weatherStatus.textContent = desc;
      weatherTemp.textContent = temp + "°C";
      weatherIcon.textContent = icon;
      weatherLoc.textContent = locName;
    }
  } catch (error) {
    console.error("Gagal mengambil data cuaca:", error);
    weatherStatus.textContent = "Gagal memuat cuaca";
  }
}
...
function updateDateTime() {
  ...
  if (greetingDisplay) {
    ...
    if (hour >= 4 && hour < 11) {
      ...
      greetingText = "Selamat Sore 🌤️";
    } else {
      greetingText = "Selamat Malam 🌙";
    }

    greetingDisplay.textContent = greetingText + ", Rekan Guru & Staff";
  }
    ...
    async (position) => {
      ...
      if (now - lastGeocodeTime > 5000) {
        ...
        try {
          ...
          // Get a compact area label for the weather widget
          cityArea = data.address.village || data.address.suburb || data.address.town || data.address.city || "Lokasi Anda";
        } catch (err) {
          currentLocation.address = "Alamat tidak dapat diambil (Gunakan koordinat)";
        }
      } else if (!currentLocation.address) {
        currentLocation.address = "Menggunakan koordinat GPS";
      }
      ...
      // Refresh weather directly with precise current GPS location!
      fetchWeather(currentLocation.lat, currentLocation.lng, cityArea);
